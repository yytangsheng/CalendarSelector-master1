...
public class CalendarUtils {
	 ...
	 * @return the days of month
	 */
	public static ArrayList<Day> getDaysOfMonth(Calendar calendar, int passDays, List<String> orderDay) {
		...
		for (int i = j - 1; i < days.size(); i++) {
			...
			if(orderDay != null){
				//当前这天的日期
				String currentDay = month.get(Calendar.YEAR) + "#" + (month.get(Calendar.MONTH) + 1 + "#")  + dayNumber;
				if(orderDay.contains(currentDay)){
					days.set(i, new Day("" + dayNumber, type, true));
					Log.e("tag","包含了" + currentDay);
				}else{
					days.set(i, new Day("" + dayNumber, type, false));
				}
			}
